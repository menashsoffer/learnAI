...
export function SceneView({ scene }: { scene: SceneRecord }) {
  ...
  const api = useSceneApi(scene.slug);
...
/** Off-window scenes render nothing until they enter the active +/- 1 mount window. */
export function ScenePlaceholder(_props: { scene: SceneRecord }) {
  return null;
}
